...
CLOUD_INIT_NN="./cloud-init/cloud-config-nn.yaml"
CLOUD_INIT_SNAP="./cloud-init/cloud-config-snap.yaml"
CLOUD_INIT_SNAP_NN="./cloud-init/cloud-config-snap-nn.yaml"
...
if [[ "$NO_NEST" == "true" ]]; then
    ...
    echo -e "${BYELLOW}-> 2. Cleaning up LXD project: $VM_NAME... ${NC}"
    echo ""
    if lxc project list | grep -q " $VM_NAME "; then
        # Delete all instances inside the $VM_NAME project first
        echo "Stopping and deleting all instances in project '$VM_NAME'..."
        for inst in \$(lxc list --project $VM_NAME --format json | jq -r '.[].name'); do
            lxc delete -f --project $VM_NAME "\$inst"
        done
        
        # Switch to default to ensure we aren't "inside" the project we are deleting
        lxc project switch default >/dev/null 2>&1
        lxc project delete $VM_NAME
        echo ""
    else
        echo "LXD project '$VM_NAME' not found, skipping..."
        echo ""
    fi
fi
