...
CLOUD_INIT="./cloud-init/cloud-config-server.yaml"
CLOUD_INIT_CLIENT="./cloud-init/cloud-config-client.yaml"
CLOUD_INIT_HA="./cloud-init/cloud-config-ha.yaml"
...
if [ "$HA_MODE" == "false" ]; then
    ...
    HAPROXY_IP=$(ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "ubuntu@$IP" "juju exec --unit haproxy/leader -- unit-get public-address" 2>/dev/null)
