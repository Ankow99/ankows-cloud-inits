...
if [ "$DEVSTACK_MODE" == "false" ]; then
    ...
    PHASE="LOG_TAIL" 
    JUJU_TRIGGER_MSG="-------- Creating Juju Openstack Model... --------"
    ...
    # 7. Open Horizon dashboard in browser
    echo ""
    echo -e "${BYELLOW}-> 7. Opening Horizon Dashboard... ${NC}"

    # Open Browser
    if command -v xdg-open &> /dev/null; then
        xdg-open "http://$DASHBOARD_IP/horizon/" > /dev/null 2>&1 &
    fi
...
EOF
    ...
    echo ""
    echo -e "${BYELLOW}-> 4. Opening Horizon Dashboard... ${NC}"
